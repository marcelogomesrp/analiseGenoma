...
@Named(value = "VfClinvarDisease")
@RequestScoped
public class VfClinvarDisease {
    ...
    @Inject
    private ClinvarDiseaseService service;
...
//        if (idAnalise != null) {
        try {
            ...
            List<String> target = filtro.getClinvardiseases().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getClinvarDiseases().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
            list = new DualListModel<>(source, target);
        } catch (Exception ex) {
            System.out.println("VfClinvarDisease.init Erro: " + ex);
        }
    }
//    }
    ...
    private void updateFiltro() {
        Set<ClinvarDisease> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<ClinvarDisease> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
