...
@Named(value = "VfGenotype")
@RequestScoped
public class VfGenotype {
    ...
    public void init() {
        try {
            ...
            List<String> target = filtro.getGenoTypes().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getGenoTypies().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
            list = new DualListModel<>(source, target);
        } catch (Exception ex) {
            System.out.println("VfGenotype.init Erro: " + ex);
        }
    }
    ...
    private void updateFiltro() {
        Set<GenoType> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<GenoType> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
