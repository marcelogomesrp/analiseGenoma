...
@Named(value = "VfMutationTaster")
@RequestScoped
public class VfMutationTaster {
    ...
    @Inject
    private MutationTasterService service;
    ...
    public void init() {
        try {
            ...
            List<String> target = filtro.getMutationTasters().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getMutationTasters().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
            list = new DualListModel<>(source, target);
        } catch (Exception ex) {
            System.out.println("VfPolyphenHdiv.init Erro: " + ex);
        }
    }
    ...
    private void updateFiltro() {
        Set<MutationTaster> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<MutationTaster> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
