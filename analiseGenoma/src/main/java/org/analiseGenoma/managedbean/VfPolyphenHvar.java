...
@Named(value = "VfPolyphenHvar")
@RequestScoped
public class VfPolyphenHvar {
    ...
    @Inject
    private PolyphenHvarService service;
    ...
    public void init() {
        try {
            ...
            List<String> target = filtro.getPolyphenHvars().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getPolyphenHvars().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
            list = new DualListModel<>(source, target);
        } catch (Exception ex) {
            System.out.println("VfPolyphenHvar.init Erro: " + ex);
        }
    }
    ...
    private void updateFiltro() {
        Set<PolyphenHvar> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<PolyphenHvar> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
                listRef.add(tmp.get(0));
            }
        }
        filtro.setByType(!listRef.isEmpty());
        filtro.setPolyphenHvars(listRef);
    }
}
