...
@Named(value = "VfPolyphenHdiv")
@RequestScoped
public class VfPolyphenHdiv {
    ...
    public void init() {
        try {
            ...
            List<String> target = filtro.getPolyphenHdivs().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getPolyphenHdivs().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
    ...
    private void updateFiltro() {
        Set<PolyphenHdiv> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<PolyphenHdiv> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
                listRef.add(tmp.get(0));
            }
        }
        filtro.setByType(!listRef.isEmpty());
        filtro.setPolyphenHdivs(listRef);
    }
}
