...
public class VfSift {
    ...
    public void init() {
        try {
            ...
            List<String> target = filtro.getSifts().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getSifts().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
            list = new DualListModel<>(source, target);
        } catch (Exception ex) {
            System.out.println("VfSift.init Erro: " + ex);
        }
    }
    ...
    private void updateFiltro() {
        Set<Sift> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<Sift> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
