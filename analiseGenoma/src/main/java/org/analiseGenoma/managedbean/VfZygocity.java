...
@Named(value = "VfZygocity")
@RequestScoped
public class VfZygocity {
    ...
    public void init() {
        ...
        if (idAnalise != null) {
                ...
                ///getUmdPredictors().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).collect(Collectors.toList());
    ...
    private void updateFiltro() {
        Set<Zygosity> listFilter = new HashSet<>();
        for(String name: list.getTarget()){
            List<Zygosity> find =  zygosityService.findByName(name); //umdPredictorService.findByName(name);
            if(find.size() == 1)
            listFilter.add(find.get(0));
        }
        filtro.setZygosities(listFilter);
    }
}
