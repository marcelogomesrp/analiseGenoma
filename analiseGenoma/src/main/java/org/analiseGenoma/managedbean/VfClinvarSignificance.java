...
@Named(value = "VfClinvarSignificance")
@RequestScoped
public class VfClinvarSignificance {
...
//        if (idAnalise != null) {
        try {
            ...
            List<String> target = filtro.getClinvarsignificances().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getClinvarSignificances().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
