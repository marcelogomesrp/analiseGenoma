...
@Named(value = "VfClinvarAlleleOrign")
@RequestScoped
public class VfClinvarAlleleOrign {
    ...
    @Inject
    private ClinvarAlleleOriginService service;
...
//    }
    ...
    private void updateFiltro() {
        Set<ClinvarAlleleOrigin> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<ClinvarAlleleOrigin> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
