...
public class VfLrt {
    ...
    public void init() {
        try {
            ...
            List<String> target = filtro.getLrts().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getLrts().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
            list = new DualListModel<>(source, target);
        } catch (Exception ex) {
            System.out.println("VfLrt.init Erro: " + ex);
        }
    }
    ...
    private void updateFiltro() {
        Set<Lrt> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<Lrt> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
                listRef.add(tmp.get(0));
            }
        }
//        filtro.setByType(!listRef.isEmpty());
        filtro.setLrts(listRef);
    }
}
