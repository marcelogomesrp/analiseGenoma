...
@Named(value = "VfVariantStatus")
@RequestScoped
public class VfVariantStatus {
    ...
    @Inject
    private VariantStatusService service;
    ...
    public void init() {
        try {
            ...
            List<String> target = filtro.getVariantStatuses().stream().map(u -> u.getName()).sorted().collect(Collectors.toList());
            List<String> source = vcfMetadata.getVariantStatus().stream().map(u -> u.getName()).filter(u -> !target.contains(u)).sorted().collect(Collectors.toList());
            list = new DualListModel<>(source, target);
        } catch (Exception ex) {
            System.out.println("VfVariantStatus.init Erro: " + ex);
        }
    }
    ...
    private void updateFiltro() {
        Set<VariantStatus> listRef = new HashSet<>();
        for (String ref : list.getTarget()) {
            List<VariantStatus> tmp = service.findByName(ref);
            if (!tmp.isEmpty()) {
                listRef.add(tmp.get(0));
            }
        }
        filtro.setByType(!listRef.isEmpty());
        filtro.setVariantStatuses(listRef);
    }
}
